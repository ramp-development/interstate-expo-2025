...
(() => {
  ...
  var _deWordify = function _deWordify2(e) {
    var children2 = _toArray2(e.childNodes), l = children2.length, i, child2;
    for (i = 0; i < l; i++) {
      child2 = children2[i];
      if (child2._isSplit) {
        _deWordify2(child2);
    ...
    if (lines2 || absolute) {
      for (i = 0; i < l; i++) {
        ...
        if (isChild || absolute || chars && !words) {
          ...
          if (lines2 && isChild && Math.abs(offset - lineOffsetY) > lineThreshold && (node.nodeName !== "BR" || i === 0)) {
            curLine = [];
            lines2.push(curLine);
            lineOffsetY = offset;
          }
  ...
  var _splitRawText = function _splitRawText2(element, vars, wordStart, charStart) {
    var tag = vars.tag ? vars.tag : vars.span ? "span" : "div", types = vars.type || vars.split || "chars,words,lines", chars = ~types.indexOf("chars"), absolute = _isAbsolute(vars), wordDelimiter = vars.wordDelimiter || " ", space = wordDelimiter !== " " ? "" : absolute ? "&#173; " : " ", wordEnd = "</" + tag + ">", wordIsOpen = 1, specialChars = vars.specialChars ? typeof vars.specialChars === "function" ? vars.specialChars : _findSpecialChars : null, text, splitText, i, j, l, character, hasTagStart, testResult, container = _doc5.createElement("div"), parent = element.parentNode;
  ...
  var _split = function _split2(element, vars, wordStart, charStart) {
    var children2 = _toArray2(element.childNodes), l = children2.length, absolute = _isAbsolute(vars), i, child2;
  ...
  // node_modules/.pnpm/@studio-freight+lenis@1.0.33/node_modules/@studio-freight/lenis/dist/lenis.mjs
  function t(t2, e, i) {
  ...
  var cards = () => {
    ...
    const components2 = queryElements(`[${attr}="component"]`);
    resizeCards();
  ...
  var countdown = () => {
    ...
    if (!component)
      return;
    const days = queryElement(`[${attr}="days"]`, component);
    const hours = queryElement(`[${attr}="hours"]`, component);
    const minutes = queryElement(`[${attr}="minutes"]`, component);
    const seconds = queryElement(`[${attr}="seconds"]`, component);
    if (!days || !hours || !minutes || !seconds)
      return;
    const target = "December 31 2024 06:59:59 UTC+9";
    const second = 1e3, minute = second * 60, hour = minute * 60, day = hour * 24;
    const countDown = new Date(target).getTime(), tick = setInterval(function() {
  ...
  var gallery = () => {
    ...
    if (!component)
      return;
    const trigger = queryElement(".w-lightbox", component);
    const button = queryElement(".btn_main_wrap", component);
    button.addEventListener("click", () => {
      simulateEvent(trigger, "click");
    });
  };

  // src/components/lines.ts
  var lines = () => {
    console.log("lines");
    const attr = "data-lines";
    const triggers = queryElements(`[${attr}="trigger"]`);
    if (triggers.length === 0)
      return;
    triggers.forEach((trigger) => {
      const lines2 = queryElement(`[${attr}="target"]`, trigger);
      const icon = queryElement(`[${attr}="icon"]`, trigger);
      if (!lines2)
        return;
      const split = new SplitText(lines2, {
        type: "lines, words",
      ...
      if (icon)
        timeline2.from(icon, { opacity: 0, rotateZ: 20 });
      split.lines.forEach((line, index) => {
        const el = line;
        el.style.overflow = "hidden";
        timeline2.from(line.children, { yPercent: 100 }, index === 0 ? 0 : "<0.2");
      });
    });
  };
  ...
  var sliders = () => {
    ...
    const components2 = queryElements(".splide");
    components2.forEach((component) => {
      const name = component.classList.contains("is-gallery") ? "gallery" : component.classList.contains("is-transformation") ? "transformation" : component.classList.contains("is-gigaprojects") ? "gigaprojects" : "failed";
      const options = {
        pagination: false,
        gap: 0,
      ...
      if (!component.parentElement)
        return;
      const rect2 = component.parentElement.getBoundingClientRect();
      const prevArrow = queryElement(".splide__arrow--prev", component);
      const nextArrow = queryElement(".splide__arrow--next", component);
